...
const acceptRegex = /app\.post\("\/api\/v1\/orders\/:id\/accept", async \(req, res\) => \{[\s\S]*?\}\);\napp\.patch/g;
if (content.match(acceptRegex)) {
  content = content.replace(acceptRegex, newAccept + '\napp.patch');
  fs.writeFileSync('server.ts', content, 'utf8');
} else {
  console.log("Could not find POST /api/v1/orders/:id/accept");
}
