const { initializeApp: initClientApp } = require('firebase/app');
const { getAuth: getClientAuth, signInWithCustomToken, getIdToken } = require('firebase/auth');
const admin = require('firebase-admin');
require('dotenv').config();

const firebaseConfig = {
  apiKey: process.env.VITE_FIREBASE_API_KEY,
  authDomain: process.env.VITE_FIREBASE_AUTH_DOMAIN,
  projectId: process.env.VITE_FIREBASE_PROJECT_ID,
  storageBucket: process.env.VITE_FIREBASE_STORAGE_BUCKET,
  messagingSenderId: process.env.VITE_FIREBASE_MESSAGING_SENDER_ID,
  appId: process.env.VITE_FIREBASE_APP_ID
};

async function getRealFirebaseToken() {
  admin.initializeApp({ projectId: process.env.VITE_FIREBASE_PROJECT_ID });
  // Need service account for createCustomToken!
  // It won't work without service account credentials...
}

getRealFirebaseToken().catch(console.error);
