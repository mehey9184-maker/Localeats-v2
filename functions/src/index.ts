...
export const createOrder = onCall(
  ...
  async (request: CallableRequest<CreateOrderRequestData>): Promise<CreateOrderResponse> => {
    // 1. Authenticate Request
    if (!request.auth || !request.auth.uid) {
      throw new HttpsError(
        "unauthenticated",
        "Authentication is required to place an order. Anonymous guest sessions are supported."
      );
    }
    const userId = request.auth.uid;

    // 2. Validate Request Shape & Essential Inputs
    const data = request.data as CreateOrderRequestData;
    ...
    if (!idempotency_key || typeof idempotency_key !== "string" || !idempotency_key.trim()) {
      throw new HttpsError("invalid-argument", "A valid idempotency_key is required.");
    }

    if (!shop_id || (typeof shop_id !== "string" && typeof shop_id !== "number")) {
      throw new HttpsError("invalid-argument", "A valid shop_id is required.");
    }
    ...
    if (delivery_type !== "delivery" && delivery_type !== "collection") {
      throw new HttpsError("invalid-argument", "delivery_type must be 'delivery' or 'collection'.");
    }

    if (delivery_schedule_mode !== "standard" && delivery_schedule_mode !== "express") {
      throw new HttpsError(
        "invalid-argument",
        "delivery_schedule_mode must be 'standard' or 'express'."
      );
    }
    ...
    if (delivery_type === "delivery") {
      if (
        ...
        throw new HttpsError(
          "invalid-argument",
          "Valid delivery GPS coordinates (lat between -90 and 90, lng between -180 and 180) are required for delivery."
        );
      }
    }
    ...
    for (const item of items) {
      if (!item.menu_item_id || typeof item.menu_item_id !== "string") {
        throw new HttpsError("invalid-argument", "Each item must have a valid menu_item_id.");
      }
      if (!Number.isInteger(item.quantity) || item.quantity <= 0) {
        throw new HttpsError("invalid-argument", `Invalid quantity for item ${item.menu_item_id}. Must be positive integer.`);
      }
    }
    ...
    if (rawTip < 0) {
      throw new HttpsError("invalid-argument", "Tip amount cannot be negative.");
    }
    ...
    const result = await db.runTransaction(async (transaction: Transaction) => {
      ...
      if (existingDoc.exists) {
        ...
        if (existingData.user_id !== userId) {
          throw new HttpsError(
            "permission-denied",
            "This order key is already assigned to a different user identity."
          );
        }
        return {
          success: true,
      ...
      if (!shopDoc.exists) {
        throw new HttpsError("not-found", `Shop '${shopIdStr}' does not exist.`);
      }
      ...
      if (!isShopActive) {
        throw new HttpsError("failed-precondition", "This shop is currently inactive and cannot accept orders.");
      }
      ...
      const shopRadiusLimit = Number(
        shopData.shopRadiusLimit ??
      ...
      for (const reqItem of items) {
        ...
        if (!menuItemData && Array.isArray(shopData.menu)) {
          menuItemData = shopData.menu.find(
            (m: any) => String(m.id) === reqItem.menu_item_id
          );
        }

        // Strategy C: Embedded shop items array
        if (!menuItemData && Array.isArray(shopData.items)) {
          menuItemData = shopData.items.find(
            (m: any) => String(m.id) === reqItem.menu_item_id
          );
        }
        ...
        if (!menuItemData) {
          throw new HttpsError(
            "not-found",
            `Menu item '${reqItem.menu_item_id}' not found in shop catalog.`
          );
        }

        // Verify shop ownership
        const itemShopId = String(
          menuItemData.shop_id || menuItemData.shopId || menuItemData.store_id || shopIdStr
        );
        if (itemShopId !== shopIdStr) {
          throw new HttpsError(
            "failed-precondition",
            `Menu item '${reqItem.menu_item_id}' does not belong to shop '${shopIdStr}'.`
          );
        }
        ...
        if (!isItemAvailable) {
          throw new HttpsError(
            "failed-precondition",
            `Menu item '${menuItemData.name || reqItem.menu_item_id}' is currently unavailable.`
          );
        }
        ...
        if (authoritativeUnitPrice < 0 || isNaN(authoritativeUnitPrice)) {
          throw new HttpsError("failed-precondition", `Invalid price for item '${reqItem.menu_item_id}'.`);
        }
      ...
      if (delivery_type === "collection") {
        ...
        if (isNaN(shopLat) || isNaN(shopLng)) {
          throw new HttpsError(
            "failed-precondition",
            "Shop location coordinates are not configured for delivery."
          );
        }
        ...
        if (distance <= 3.0) {
          ...
          throw new HttpsError(
            "failed-precondition",
            `Delivery distance (${distance.toFixed(1)} km) exceeds the shop delivery limit (${shopRadiusLimit.toFixed(1)} km).`
          );
        }
      ...
      if (promo_code && typeof promo_code === "string" && promo_code.trim()) {
        ...
        if (promoData) {
          ...
          if (!isPromoActive) {
            throw new HttpsError("failed-precondition", `Promo code '${cleanCode}' is not active.`);
          }

          if (promoData.expiry_date) {
            const expiry = new Date(promoData.expiry_date);
            if (new Date() > expiry) {
              throw new HttpsError("failed-precondition", `Promo code '${cleanCode}' has expired.`);
            }
          }
          ...
          if (promoData.shop_id && String(promoData.shop_id) !== shopIdStr) {
            throw new HttpsError(
              "failed-precondition",
              `Promo code '${cleanCode}' is not valid for this shop.`
            );
          }
          ...
          if (promoData.min_order_amount && calculatedSubtotal < Number(promoData.min_order_amount)) {
            throw new HttpsError(
              "failed-precondition",
              `Minimum order subtotal of R${Number(promoData.min_order_amount).toFixed(2)} required for '${cleanCode}'.`
            );
          }
          ...
          if (cleanCode === "BICYCLE5" && delivery_type !== "delivery") {
            throw new HttpsError(
              "failed-precondition",
              "This voucher code is only valid for Delivery orders."
            );
          }
          ...
          throw new HttpsError("invalid-argument", `Promo code '${cleanCode}' is invalid.`);
        }
      }
      ...
      const authoritativeOrderData = {
        ...
        product_name:
          orderItemsSnapshot.length > 0
            ? `${orderItemsSnapshot[0].quantity}x ${orderItemsSnapshot[0].name}${
                orderItemsSnapshot.length > 1 ? ` + ${orderItemsSnapshot.length - 1} more` : ""
              }`
            : "Food Order",
